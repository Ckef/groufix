...
_groufix objects are not reentrant_. Function calls associated with the same object created with a call to `gfx_create_*` are not synchronized and __cannot__ be called concurrently from different threads. However, internal access to the same object of any type is synchronized, unless explicitly stated otherwise. For example, the `GFXDevice` object being passed to various other objects at creation is thread-safe.
