...
* _groufix will not reference count_. This means that whenever you destroy, clear or free an object with a call to the associated `gfx_destroy_*`, `gfx_*_clear` or `gfx_free_*` function, any other object may not reference this object anymore. In other words, _groufix_ will __not__ idle during destruction or clearing until all references to the object are released.

* _groufix objects are not thread-safe_. Function calls associated with the same object (or descendants of) created with a call to `gfx_create_*` or `gfx_*_init` are not synchronized and __cannot__ be called concurrently from different threads. However, such objects referencing each other __is__ internally synchronized, meaning functions associated with different objects __can always__ be called concurrently. For example, when a `GFXRenderer` references a `GFXWindow`, thread-safety is guaranteed when the two objects are accessed or modified simultaneously.
