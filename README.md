...
* _groufix objects are not thread-safe_. Function calls associated with the same core object created with a call to `gfx_create_*` or `gfx_*_init` are not synchronized and __cannot__ be called concurrently from different threads. However, such objects referencing each other __is__ internally synchronized, meaning functions associated with different objects __can always__ be called concurrently, unless explicitly stated otherwise. For example, when a `GFXRenderer` references a `GFXWindow`, thread-safety is guaranteed when the two objects are accessed or modified simultaneously.
